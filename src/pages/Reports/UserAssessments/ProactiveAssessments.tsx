...
import { getAllProAssessmentsWithoutPagination } from "../../../store/actions/report.action";
import { clearAllBlockList, clearAllHealthFacilityList, clearCaderBytypesList, cleartAllCountryList, cleartAllDistrictList, cleartAllStateList, getAllBlockList, getAllCountryList, getAllDistrictList, getAllHealthFacilityList, getAllStateList, getCaderBytypesList } from "../../../store/reducer/masterTable.reducer";
import { clearSubscribersSearch, getAllSubscribersSearch } from "../../../store/reducer/plugin.reducer";
import { getAllProAssessments } from "../../../store/reducer/report.reducer";
import { downloadCSV } from "../../../utils/functions";

const ProactiveAssessments = () => {
    const dispatch = useDispatch();
    const { loader, proAssessmentsList, reportLoader, assessmentList } = useSelector((state: IRootState) => state.report);
    ...
    const [query, setQuery] = useQueryParams({
        page: "1", limit: "25", sortBy: "", sortOrder: "", country: "", stateIds: ArrayParam,
        districtIds: ArrayParam, blockIds: ArrayParam, healthFacilityIds: ArrayParam, userCadreId: ArrayParam, fromDate: "", toDate: "", assessmentTitle: "", userIds: ArrayParam, userEmail: "", userPhoneNo: ""
    });
    const clearFilters = () => {
        setQuery({
            page: "1",
            ...
            fromDate: null, toDate: null, assessmentTitle: null, userIds: null, userEmail: null, userPhoneNo: null
        });
    };
    useEffect(() => {
        if (window.location.search === "") {
            ...
            dispatch(getAllProAssessments(window.location.search));
        }
        if (query.stateIds) {
        ...
        <div className="panel">
            <div className="flex items-center justify-between mb-5">
                <div className="flex items-center flex-wrap gap-3">
                    <CircleWithNum number={proAssessmentsList?.totalItems}
                        title={"Proactive Assessments"} />
                    <PrimaryBtn
                        title="Clear Filters"
                ...
                <div className="flex items-center flex-wrap gap-3">
                    ...
                    <PrimaryBtn
                        ...
                        onClick={() => {
                            ...
                            dispatch(getAllProAssessmentsWithoutPagination(`?${searchParams}`
                                , (res) => {
                                    downloadCSV(res, "proactive_assessments.csv");
                                }
                            ));
                        }}
                    />
            ...
            <div className="mb-5">
                <TableComponent
                    columns={[
                        { accessor: "_id", hidden: true },
                        {
                            ...
                            filter(params) {
                                return <PrimaryTextInput
                                    ...
                                    id="assessment"
                                    type="text"
                                    onChange={(e) => setQuery({ assessmentTitle: e })}
                                    value={query.assessmentTitle as string}
                                />;
                            },
                        },
                        {
                            ...
                            filter(params) {
                                return (
                                    <MultiSelectInput
                                        ...
                                        options={localMembersList?.map((member: any) => ({
                        ...
                        {
                            title: "Cadre",
                            accessor: "userId.cadre",
                            sortable: true,
                            render: (record) => {
                                return record?.userId?.cadre?.[0]?.title;
                            },
                        ...
                        {
                            title: "Country",
                            accessor: "userId.country",
                            sortable: true,
                            render: (record) => {
                                return record?.userId?.country?.[0]?.title;
                            }
                        },
                        {
                            title: "State",
                            accessor: "userId.state",
                            sortable: true,
                            render: (record) => {
                                return record?.userId?.state?.[0]?.title;
                            },
                        ...
                        {
                            title: "District",
                            accessor: "userId.district",
                            sortable: true,
                            render: (record) => {
                                return record?.userId?.district?.[0]?.title;
                            },
                        ...
                        {
                            title: "Block",
                            accessor: "userId.block",
                            sortable: true,
                            render: (record) => {
                                return record?.userId?.block?.[0]?.title;
                            },
                    ...
                    idAccessor={({ _id }) => _id}
                    records={proAssessmentsList?.list}
                    isloading={loader}
                    page={proAssessmentsList?.currentPage || undefined}
                    onPageChange={(page) => {
                    ...
                    totalRecords={proAssessmentsList?.totalItems || 0}
...
export default ProactiveAssessments;

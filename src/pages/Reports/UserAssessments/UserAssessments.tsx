...
import { getAllUserAssessmentsWithoutPagination } from "../../../store/actions/report.action";
...
import { clearAllAssessmentListWithoutPagination, getAllAssessmentListWithoutPagination, getAllUserAssessments } from "../../../store/reducer/report.reducer";
import { downloadCSV } from "../../../utils/functions";

const UserAssessments = () => {
    const dispatch = useDispatch();
    const { loader, userAssessmentsList, reportLoader, assessmentList } = useSelector((state: IRootState) => state.report);
    ...
    useEffect(() => {
        if (window.location.search === "") {
            ...
            dispatch(getAllUserAssessments(window.location.search));
        }
        if (query.stateIds) {
    ...
    useEffect(() => {
        ...
        dispatch(getAllAssessmentListWithoutPagination());
        return () => {
            ...
            dispatch(clearAllAssessmentListWithoutPagination());
        ...
        <div className="panel mt-6">
            <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                <div className="flex items-center flex-wrap gap-3">
                    <CircleWithNum number={userAssessmentsList?.totalItems}
                ...
                <div className="flex items-center flex-wrap gap-3">
                    ...
                    <PrimaryBtn
                        ...
                        onClick={() => {
                            ...
                            dispatch(getAllUserAssessmentsWithoutPagination(`?${searchParams}`
                                , (res) => {
                                    downloadCSV(res, "user_assessments.csv");
                                }
                            ));
                        }}
                    />
            ...
            <TableComponent
                ...
                columns={[
                    { accessor: "_id", hidden: true },
                    {
                        ...
                        filtering: query?.assessmentId ? true : false,
                        filter(params: { close: () => void; }) {
                            return (
                                <MultiSelectInput
                                    ...
                                    id="assessmentId"
                                    value={query?.assessmentId as Array<string>}
                                    onChange={(e) => {
                                        setQuery({ page: "1", assessmentId: e });
                                    }}
                                    options={assessmentList && assessmentList?.map((e: any) => ({
                                        label: e?.title?.en || e?.title?.gu,
                                        value: e._id
                                    }))}
                                />
                            );
                ...
                idAccessor={({ _id }) => _id}
                records={userAssessmentsList?.list}
                page={userAssessmentsList?.currentPage || undefined}
                onPageChange={(page) => {
                ...
                totalRecords={userAssessmentsList?.totalItems || 0}

...
import { StateEditForm } from "../../../forms/StateEditForm";
import { IRootState } from "../../../store";
import { deleteState } from "../../../store/actions/masterTable.action";
import { logActivity } from "../../../store/actions/report.action";
import { clearState, clearStateByID, cleartAllCountryList, getAllCountryList, getState, getStateById } from "../../../store/reducer/masterTable.reducer";
import { setPageTitle } from "../../../store/themeConfigSlice";
import { hasSinglePermission } from "../../../utils/functions";
import { ErrorToast, WarningToast } from "../../../utils/toasts";
const Index = () => {
    const [query, setQuery] = useQueryParams({ page: "", limit: "", sortOrder: StringParam, sortBy: StringParam, title: StringParam });
    const clearFilters = () => {
        setQuery({
            ...query,
            sortOrder: null,
    ...
    const { stateDetails, loader, stateById } = useSelector((state: IRootState) => state.master);
    const [opened, { open, close }] = useDisclosure(false);
    useEffect(() => {
        if (window.location.search === "") {
            setQuery({ page: "1", limit: "15" });
        } else {
            dispatch(getState(window.location.search));
        }
        dispatch(setPageTitle("State List"));
        ...
        <div className="panel mt-6">
            <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                <div className="flex items-center flex-wrap gap-3">
                    <span
                        ...
                        {stateDetails?.totalItems || 0}
                    </span>
                    <h5 className="font-semibold text-lg">State</h5>
                    <PrimaryTextInput
                        type="text"
                        id='Master-seaarch'
                ...
                <div className="flex items-center flex-wrap gap-3">
                    <AuthLayout actions={["admin.state.create"]}>
                        <PrimaryBtn
                            title="Add New"
                            onClick={open}
                            leftIcon={<IconPlus />}
            ...
            <TableComponent
                ...
                columns={[{ accessor: "_id", hidden: true },
                {
                    ...
                    accessor: "countryId", title: "Country Name", sortable: true, render: (item: any) => item?.countryId?.title,

                },
                {
                    accessor: "title", title: "State Name", sortable: true,
                },
                {
                ...
                },
                {
                    ...
                    render: (item: any) => (
                        <AuthLayout actions={["admin.state.edit", "admin.state.delete"]}
                            showDash>
                            <div className="flex items-center justify-end gap-6 mr-3 ">
                                <AuthLayout actions={["admin.state.edit"]}>
                                    <Tippy content="Edit">
                                        <button type="button"
                                            onClick={() => {
                                                dispatch(getStateById(item?._id));
                                                open();
                                            }}>
                                            <IconPencil />
                                        </button>
                                ...
                                <AuthLayout actions={["admin.state.delete"]}>
                ...
                records={stateDetails?.list}
                onRowClick={({ record }) => {
                    if (hasSinglePermission("admin.state.edit")) {
                        dispatch(getStateById(record?._id));
                        open();
                    }
                }}
                idAccessor={({ _id }) => _id}
                page={stateDetails?.currentPage || undefined}
                onPageChange={(page) => {
                ...
                totalRecords={stateDetails?.totalItems || 0}
            ...
            <DrawerModal
                ...
                onClose={() => {
                    dispatch(getState(window.location.search));
                    dispatch(clearStateByID());
                    close();
                }}
                title={
                    <span className="text-xl font-bold">{stateById?._id ? "Edit State" : "Add New               State"}</span>
                }
